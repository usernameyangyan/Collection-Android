...
import com.youngmanster.collectionlibrary.base.queue.Action;
import com.youngmanster.collectionlibrary.base.queue.ActionQueue;
...
public class TransactionDelegate {
    ...
    private BaseSupportActivity mSupport;
    ...
    TransactionDelegate(BaseSupportActivity support) {
    ...
    void loadRootTransaction(final FragmentManager fm, final int containerId, final BaseSupportFragment to, final boolean addToBackStack, final boolean allowAnimation) {
    ...
    void loadMultipleRootTransaction(final FragmentManager fm, final int containerId, final int showPosition, final BaseSupportFragment... tos) {
        enqueue(fm, new Action(Action.ACTION_LOAD) {
            @Override
            public void run() {
     ...
     */
    void dispatchStartTransaction(final FragmentManager fm, final BaseSupportFragment from, final BaseSupportFragment to, final int requestCode, final int launchMode, final int type) {
        enqueue(fm, new Action(launchMode == BaseSupportFragment.SINGLETASK ? Action.ACTION_POP_MOCK : Action.ACTION_NORMAL) {
            @Override
            public void run() {
     ...
     */
    void showHideFragment(final FragmentManager fm, final BaseSupportFragment showFragment, final BaseSupportFragment hideFragment) {
     ...
     */
    void startWithPop(final FragmentManager fm, final BaseSupportFragment from, final BaseSupportFragment to) {
        enqueue(fm, new Action(Action.ACTION_POP_MOCK) {
            @Override
            public void run() {
                BaseSupportFragment top = getTopFragmentForStart(from, fm);
                if (top == null)
        ...
        dispatchStartTransaction(fm, from, to, 0, BaseSupportFragment.STANDARD, TransactionDelegate.TYPE_ADD);
    }

    void startWithPopTo(final FragmentManager fm, final BaseSupportFragment from, final BaseSupportFragment to, final String fragmentTag, final boolean includeTargetFragment) {
        enqueue(fm, new Action(Action.ACTION_POP_MOCK) {
            @Override
            public void run() {
                int flag = 0;
                ...
                final BaseSupportFragment top = getTopFragmentForStart(from, fm);
                if (top == null)
                    throw new NullPointerException("There is no Fragment in the FragmentManager, maybe you need to call loadRootFragment() first!");
    ...
    void remove(final FragmentManager fm, final Fragment fragment, final boolean showPreFragment) {
        enqueue(fm, new Action(Action.ACTION_POP, fm) {
            ...
            public void run() {
                ...
                if (showPreFragment) {
                    BaseSupportFragment preFragment = SupportHelper.getPreFragment(fragment);
    ...
    private void removeTopFragment(FragmentManager fm) {
        try { // Safe popBackStack()
            BaseSupportFragment top = SupportHelper.getBackStackTopFragment(fm);
            if (top != null) {
                fm.beginTransaction()
                        .setTransition(FragmentTransaction.TRANSIT_FRAGMENT_CLOSE)
                        .remove((Fragment) top)
     ...
     */
    boolean dispatchBackPressedEvent(BaseSupportFragment activeFragment) {
        if (activeFragment != null) {
            ...
            if (dispatchBackPressedEvent((BaseSupportFragment) parentFragment)) {
                return true;
            }
        }
        return false;
    ...
    void handleResultRecord(Fragment from) {
        try {
            ...
            BaseSupportFragment targetFragment = (BaseSupportFragment) from.getFragmentManager().getFragment(from.getArguments(), FRAGMENTATION_STATE_SAVE_RESULT);
    ...
    private void doDispatchStartTransaction(FragmentManager fm, BaseSupportFragment from, BaseSupportFragment to, int requestCode, int launchMode, int type) {
    ...
    private BaseSupportFragment getTopFragmentForStart(BaseSupportFragment from, FragmentManager fm) {
        BaseSupportFragment top;
        if (from == null) {
    ...
    private void start(FragmentManager fm, final BaseSupportFragment from, BaseSupportFragment to, String toFragmentTag,
    ...
    private void doShowHideFragment(FragmentManager fm, BaseSupportFragment showFragment, BaseSupportFragment hideFragment) {
    ...
    private void bindContainerId(int containerId, BaseSupportFragment to) {
    ...
    private boolean handleLaunchMode(FragmentManager fm, BaseSupportFragment topFragment, final BaseSupportFragment to, String toFragmentTag, int launchMode) {
        if (topFragment == null) return false;
        final BaseSupportFragment stackToFragment = SupportHelper.findBackStackFragment(to.getClass(), toFragmentTag, fm);
        if (stackToFragment == null) return false;

        if (launchMode == BaseSupportFragment.SINGLETOP) {
            if (to == topFragment || to.getClass().getName().equals(topFragment.getClass().getName())) {
                handleNewBundle(to, stackToFragment);
                return true;
            }
        } else if (launchMode == BaseSupportFragment.SINGLETASK) {
    ...
    private void handleNewBundle(BaseSupportFragment toFragment, BaseSupportFragment stackToFragment) {
    ...
    private void mockPopToAnim(Fragment from, String targetFragmentTag, FragmentManager fm, int flag, List<Fragment> willPopFragments, int popAnim) {
        if (!(from instanceof BaseSupportFragment)) {
            safePopTo(targetFragmentTag, fm, flag, willPopFragments);
            return;
        }

        final BaseSupportFragment fromSupport = (BaseSupportFragment) from;
    ...
    private void mockStartWithPopAnim(final BaseSupportFragment from, BaseSupportFragment to, final Animation exitAnim) {
